...
async function prepareInputs() {
    ...
    try {
        ...
        const dataArray = getArrayFromNoirInput(noirInputs.data);
        const dataLen = dataArray ? dataArray.length : 0; // Determine actual length
        // Use the actual length determined above if noir-jwt doesn't provide it directly
        const actualDataLen = noirInputs.data && typeof noirInputs.data === 'object' && 'len' in noirInputs.data 
                                ? noirInputs.data.len 
                                : dataLen; 
        // Format as BoundedVec structure: { storage: [...], len: ... }
        tomlContent += `data_storage = ${formatHexArray(dataArray)}\n`;
        tomlContent += `data_len = ${actualDataLen}\n`;
        ...
        const expectedNationalityBytes = Array.from(Buffer.from(expectedNationalityValue, 'utf-8'));
        // Pad expected nationality to MAX_CLAIM_LENGTH (defined in Noir - assumed 100 here)
        const MAX_CLAIM_LENGTH = 100;
        const expectedNationalityContents = Array(MAX_CLAIM_LENGTH).fill(0);
        expectedNationalityBytes.forEach((byte, index) => { expectedNationalityContents[index] = byte; });

        tomlContent += `\n# Expected claim values (public inputs)\n`;
        // Use storage/len field names
        tomlContent += `expected_nationality_storage = ${formatHexArray(expectedNationalityContents)}\n`;
        tomlContent += `expected_nationality_len = ${expectedNationalityBytes.length}\n`;

        // Define output path relative to project root (app -> circuit/Prover.toml)
        const proverTomlPath = path.resolve(process.cwd(), '..', 'circuit', 'Prover.toml');
