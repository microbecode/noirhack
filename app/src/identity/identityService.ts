...
// Remove unused Resolver types
// import { Resolver, DIDResolver, DIDResolutionResult, DIDDocument, VerificationMethod } from 'did-resolver';
// import { getResolver } from 'web-did-resolver';
import { SignJWT, importJWK } from 'jose'; // Removed unused JWK type import
import { webcrypto, KeyObject } from 'node:crypto'; // Import KeyObject, keep webcrypto if needed elsewhere, remove createHash
import type { JWK as JWKType } from 'jose'; // Keep type import separate if needed
import * as fs from 'node:fs';
import * as path from 'node:path';
// Add url import for ES Module path handling
import { fileURLToPath } from 'node:url'; 

console.log("identityService.ts: Imports finished.");

// Polyfill global crypto if needed (Important for jose in Node)
...
// Correct path relative to the *source* file location (src/identity/identityService.ts)
// Needs to go up one level to `src/` then into `config/`
...
let issuerPrivateKeyPromise: Promise<KeyObject>; // Use KeyObject
try {
    if (!issuerKid) { 
    ...
    // Use static function
    issuerPrivateKeyPromise = importJWK(loadedPrivateJwk, 'RS256') as Promise<KeyObject>; // Use KeyObject
...
// --- Exports ---
// Ensure getPrivateKey and issuerKid are exported along with issuerDid
export { issueIdentityCredential, issuerDid, issuerKid, getPrivateKey }; 